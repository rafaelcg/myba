type TicketType = 'bug' | 'feature' | 'epic' | 'task' | 'improvement';
...
const TICKET_TYPE_KEYWORDS: Record<TicketType, string[]> = {
  bug: [
...
function analyzeInput(input: string): PromptContext {
  const lowerInput = input.toLowerCase();
  const words = lowerInput.split(/\s+/);

  let ticketType: TicketType = 'task';
  let maxMatches = 0;
...
function getTypeSpecificInstructions(type: TicketType, urgency: string, complexity: string): string {
  const instructions: Record<TicketType, string> = {
    bug: `
Focus on:
...
export function buildEnhancedPrompt(input: string): string {
